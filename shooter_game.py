...
mixer.init()
mixer.music.load('space-sound-hi-109577.mp3')
mixer.music.play()
window_w = 700
window_h = 500
FPS = 60
...
m = transform.scale(image.load('enemy_png.png'),(40,40))
#задай фон сцены
...
class Player(GameSprite):
    def update(self):
        ...
        if keys_pressed[K_RIGHT]and self.rect.x<window_w-60:
            self.rect.x += self.speed
        if keys_pressed[K_UP] and self.rect.y>0 and keys_pressed[K_HOME]:
            self.rect.y -= self.speed
        if keys_pressed[K_DOWN] and self.rect.y<window_h-60 and keys_pressed[K_HOME]:
            self.rect.y += self.speed
class Enemy(GameSprite):
    def update(self):
        ...
        if self.rect.y >= window_h:
            ...
            missed = missed + 1
            missed_text = font1.render(f'Пропущено: {missed}',True,(255,255,255))
class EN(GameSprite):
    def update(self):
        self.rect.y += self.speed
        global missed,missed_text
...
for i in range(5):
    m = Enemy('enemy_png.png',randint(1,3),randint(0,650),0,65,65)
    monsters.add(m)
missed = 0
shot = 0
font.init()
...
missed_text = font1.render(f'Пропущено:{missed}',True,(255,255,255))
shot_text = font1.render(f'Счет:{shot}',True,(255,255,255))    
while game:
    for e in event.get():
        if e.type == QUIT:
            game = False
    ...
    if not finish:
        ...
        for i in bullets_enemys:
            m = Enemy('enemy_png.png',randint(2,4),randint(0,650),0,65,65)
            shot += 1
            monsters.add(m)
            shot_text = font1.render(f'Счет:{shot}',True,(255,255,255))
        ...
        finish_text = font2.render(f'Проигрыш',True,(255,255,255))
